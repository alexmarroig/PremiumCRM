...
- `POST /api/v1/automations/debug/sign` (debug assinatura, admin + flag)
...
- Segurança: valida assinatura (HMAC-SHA256), timestamp, tenant ownership, destination habilitado.
- Payload suportado:
  - `tenant_id`, `correlation_id|event_id`, `action`, `params|payload`
- Base string assinatura (especificação única): `timestamp.event_id.tenant_id.body_json_minificado`
- Ações MVP:
  - `create_task`
...
3. **Routers**
   ...
   - Endpoint debug de assinatura `/api/v1/automations/debug/sign` guardado por flag+admin e desabilitado em produção.
